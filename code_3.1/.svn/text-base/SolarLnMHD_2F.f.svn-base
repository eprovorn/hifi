...
c-----------------------------------------------------------------------
     ...
     $     Tph=1.,Tco=1.,p0=1.,y_eta=1.,y0_eta=0.,mu_min=0.
...
c-----------------------------------------------------------------------
      ...
      REAL(r8), DIMENSION(SIZE(y,1),SIZE(y,2)) :: lx_loc
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
     ...
     $     + eta_vbl*EXP(-((x-half*lx_loc)/0.125)**2)
...
c-----------------------------------------------------------------------
      ...
      mu_local = rho*(mu + mu_vbl*EXP(-((x-half*lx_loc)/0.1)**2)
...
c-----------------------------------------------------------------------
     ...
     $     - vi(2,:,:)*(uy(8,:,:)+py0) - hexch_un + eta_rho*jtot**2)
      ...
      s_u(8,8,:,:)=r_fac*(eta_p*jtot**2 - hexch_pe)
      s_u(8,9,:,:)=-r_fac*hexch_pi
...
c-----------------------------------------------------------------------
     ...
     $        /(.125*x_curve + .5)
      ...
      CASE("Chen-Shibata-hlf","CS-stratified")
        x=lx*0.5*(x_curve*ksi**3 + ksi)/(x_curve + 1.)
        y=ly*(y_curve*phi**2 + phi)/(y_curve+one)
      CASE("CurrentSheet")
        y=ly*(y_curve*phi**2 + phi)/(y_curve+one)
        x=lx*(one + 0.1*y)*0.5*(x_curve*ksi**3 + ksi)/(x_curve + 1.)
      END SELECT
c-----------------------------------------------------------------------
c     terminate.
c-----------------------------------------------------------------------
