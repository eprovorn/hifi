c-----------------------------------------------------------------------
c     file mhd7.f.
c     contains specifications for Hall MHD model with 7 variables.
c-----------------------------------------------------------------------
c-----------------------------------------------------------------------
c     module organization.
c-----------------------------------------------------------------------
c     subprogram 0. mhd7_mod.
c     subprogram 1. mhd7_bubble.
c     subprogram 2. mhd7_cross.
c     subprogram 3. mhd7_eig.
c-----------------------------------------------------------------------
...
c     declarations.
c-----------------------------------------------------------------------
      SUBROUTINE mhd7_bubble(key,index,mmin,mmax)
...
c     declarations.
c-----------------------------------------------------------------------
      FUNCTION mhd7_cross(a,b) RESULT(c)
...
c     declarations.
c-----------------------------------------------------------------------
      SUBROUTINE mhd7_eig(kvec,e,kpar,kprp,evals,omega,v,b,p,jz,
     $     init_type,jeig,errmax)
      ...
      COMPLEX(r8), DIMENSION(3), INTENT(OUT) :: p,jz
...
c-----------------------------------------------------------------------
      IF(hall_flag)THEN
         ...
         DO j=1,3
            rmat(:,j)=mhd7_cross(kvec,rmat(:,j))
         ENDDO
         cmat(2:4,2:4)=-ifac*rmat
      ENDIF
c-----------------------------------------------------------------------
c     compute hyperresistivity terms.
c-----------------------------------------------------------------------
      cmat(2,2)=cmat(2,2)-ifac*nu*k0**4
      cmat(3,3)=cmat(3,3)-ifac*nu*k0**4
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      index=(/(i,i=1,nqty)/)
      key=wvec
      CALL mhd7_bubble(key,index,1,nqty)
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      DO i=1,3
         ...
         b(:,i)=vr(2:4,index(i))
         v(:,i)=vr(5:7,index(i))
         jz(i)=ifac*(kvec(1)*b(2,i)-kvec(2)*b(1,i))
      ENDDO
      evals=omega**2
c-----------------------------------------------------------------------
c     select eigenpair.
...
c-----------------------------------------------------------------------
     ...
     $        /MAXVAL(ABS(vr(:,i)))
      ENDDO
      errmax=MAXVAL(error)
c$$$      WRITE(*,'(a,1p,e9.3)')" errmax = ",errmax
c-----------------------------------------------------------------------
c     terminate.
c-----------------------------------------------------------------------
      RETURN
      END SUBROUTINE mhd7_eig
      END MODULE mhd7_mod
c-----------------------------------------------------------------------
c     subprogram a. physics_input.
...
c-----------------------------------------------------------------------
      e(:,1)=(/Bx,By,Bz/)
      e(:,2)=mhd7_cross(kvec,e(:,1))
      e(:,3)=mhd7_cross(e(:,1),e(:,2))
      DO i=1,3
...
c     compute eigenvalues and eigenvectors.
c-----------------------------------------------------------------------
      CALL mhd7_eig(kvec,e,kpar,kprp,evals,omega,v,b,p,jz,init_type,
     $     jeig,errmax)
c-----------------------------------------------------------------------
c     compute u0fac.
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      ...
      OPEN(UNIT=debug_unit,FILE="mhd7.out",STATUS="UNKNOWN")
      WRITE(debug_unit,10)knx,kny,hall_flag,hall_type,jeig,
     $     TRIM(init_type),thetaB,phiB,beta0,di
...
c-----------------------------------------------------------------------
      ...
      CALL MPI_Bcast(hall_type,1,MPI_INTEGER,0,comm,ierr)
c-----------------------------------------------------------------------
...
c     vx.
c-----------------------------------------------------------------------
      fx(5,:,:)=u(1,:,:)+ux(2,:,:)*By+u(3,:,:)*Bz
      fy(5,:,:)=uy(2,:,:)*By
c-----------------------------------------------------------------------
c     vy.
c-----------------------------------------------------------------------
      fx(6,:,:)=-ux(2,:,:)*Bx
      fy(6,:,:)=u(1,:,:)-uy(2,:,:)*Bx+u(3,:,:)*Bz
c-----------------------------------------------------------------------
c     vz.
c-----------------------------------------------------------------------
      fx(7,:,:)=-u(3,:,:)*Bx
      fy(7,:,:)=-u(3,:,:)*By
c-----------------------------------------------------------------------
c     compute Hall terms.
c-----------------------------------------------------------------------
      IF(hall_flag .AND. hall_type == 1)THEN
         s(2,:,:)=s(2,:,:)+(ux(3,:,:)*Bx+uy(3,:,:)*By)/omci
         fx(3,:,:)=fx(3,:,:)+u(4,:,:)*Bx/omci
         fy(3,:,:)=fy(3,:,:)+u(4,:,:)*By/omci
      ENDIF
c-----------------------------------------------------------------------
c     add hyperresistivity.
c-----------------------------------------------------------------------
      fx(2,:,:)=nu*ux(4,:,:)
      fy(2,:,:)=nu*uy(4,:,:)
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      IF(hall_flag .AND. hall_type == 2)THEN
         mass(2,7,:,:)=-1/omci
         mass_x(3,6,:,:)=1/omci
         mass_y(3,5,:,:)=-1/omci
      ENDIF
