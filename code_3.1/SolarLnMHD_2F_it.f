     ...
     $            *half*(tfrac+1./twopi*SIN(twopi*(tfrac-half)))
      ...
      CASE("Chen-Shibata-hlf") 
         ...
         CASE ("right")
            ...
            c(5,:,:)=ux(5,:,:)-u(5,:,:)*ux(1,:,:)
            c(6,:,:)=ux(6,:,:)-u(6,:,:)*ux(1,:,:)
            c(7,:,:)=nhat(1,:,:)*ux(7,:,:)+nhat(2,:,:)*uy(7,:,:)
         CASE("top")
            c(1,:,:)=nhat(1,:,:)*ux(1,:,:)+nhat(2,:,:)*uy(1,:,:) ! 0=grad_n(u)
            c(3,:,:)=nhat(1,:,:)*ux(3,:,:)+nhat(2,:,:)*uy(3,:,:)
            c(4,:,:)=u(4,:,:)
            c(5,:,:)=nhat(1,:,:)*ux(5,:,:)+nhat(2,:,:)*uy(5,:,:)
            c(6,:,:)=u(6,:,:)
            c(7,:,:)=nhat(1,:,:)*ux(7,:,:)+nhat(2,:,:)*uy(7,:,:)
         CASE("bottom")
            psi_bc=SolarLnMHD_psi(x,y)
...
c Elena: (p_i+p_0) = n*(T_i+T_0)
      Tex = zero
c      rho_inv*(ux(8,:,:)+px0 - (u(8,:,:)+pt0)*ux(1,:,:)) - Tx0
      Tey = zero
c      rho_inv*(uy(8,:,:)+py0 - (u(8,:,:)+pt0)*uy(1,:,:)) - Ty0
      Tix = zero
c      rho_inv*(ux(9,:,:)+px0 - (u(9,:,:)+pt0)*ux(1,:,:)) - Tx0
      Tiy = zero
c      rho_inv*(uy(9,:,:)+py0 - (u(9,:,:)+pt0)*uy(1,:,:)) - Ty0
c-----------------------------------------------------------------------
...
c Elena default case?
     ...
     $        u(8,:,:)+pt0,jtot,0._r8,etas_norm,0._r8,0._r8,0._r8,
     $        eta,etavac,eta_local)
      CASE("spitzer-chodura")
         CALL transport_seteta(eta_case,r_sphr,rho,u(8,:,:)+pt0,jtot,
...
c     heat exchange.
c-----------------------------------------------------------------------
c     CALL transport_hexch(ieheat,ke_norm,rho,u(9,:,:)+pt0,u(8,:,:)+pt0,
c     $     heat_exch)
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
     ...
     $     + u(3,:,:)*bz0 - two*mu_local*viy(2,:,:))   
      ...
      s(5,:,:) = r_fac*(j3*b1 - 0.5_r8*beta0*rho*uy(1,:,:)   
...
c      rho_inv*(ux(8,:,:)+px0 - (u(8,:,:)+pt0)*ux(1,:,:)) - Tx0
      Tey = zero
c      rho_inv*(uy(8,:,:)+py0 - (u(8,:,:)+pt0)*uy(1,:,:)) - Ty0
c      Te_un=-(u(8,:,:)+pt0)*rho_inv
      Tix = zero
c      rho_inv*(ux(9,:,:)+px0 - (u(9,:,:)+pt0)*ux(1,:,:)) - Tx0
      Tiy = zero
c      rho_inv*(uy(9,:,:)+py0 - (u(9,:,:)+pt0)*uy(1,:,:)) - Ty0
c      Ti_un=-(u(9,:,:)+pt0)*rho_inv
c-----------------------------------------------------------------------
c     magnetic fields and currents.
c-----------------------------------------------------------------------
      b1 = -(cyl_fac*r_faci*u(2,:,:) + uy(2,:,:))
...
c-----------------------------------------------------------------------
     ...
     $        u(8,:,:)+pt0,jtot,0._r8,etas_norm,0._r8,0._r8,0._r8,eta,
     $        etavac,eta_local,eta_rho,eta_p,eta_j)
         eta_rho = eta_rho*rho
      CASE("spitzer-chodura")
         CALL transport_seteta_u(eta_case,r_sphr,rho,u(8,:,:)+pt0,jtot,
     $        chod_const,etas_norm,etac_norm,v_chod_norm,0._r8,
...
c     heat exchange.
c-----------------------------------------------------------------------
c      CALL transport_hexch_u(ieheat,ke_norm,rho,u(9,:,:)+pt0,
c     $     u(8,:,:)+pt0,heat_exch,hexch_un,hexch_pi,hexch_pe)
c      hexch_un = hexch_un*rho
c-----------------------------------------------------------------------
c     viscous boundary layer.
c-----------------------------------------------------------------------
      SELECT CASE(init_type)
...
c-----------------------------------------------------------------------      
     ...
     $     - two*mu_min*viy(2,:,:)) 
      ...
      s_u(5,1,:,:)=-r_fac*gy_vec*rho 
     $    - r_fac*0.5_r8*beta0*rho*uy(1,:,:)
     $    - cyl_fac*(u(6,:,:)*vi(3,:,:) 
     $    - two*r_faci*mu_min*vi(2,:,:))
      ...
      s_u(5,7,:,:)=r_fac*b1
      s_uy(5,1,:,:)= - r_fac*0.5_r8*beta0*rho
      s_uy(5,2,:,:)= - r_fac*j3
c      s_uy(5,8,:,:)=-r_fac
c      s_uy(5,9,:,:)=-r_fac
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      ...
      mass(5,5,:,:)=r_fac
      mass(6,6,:,:)=r_fac
c      mass(8,8,:,:)=r_fac/(gamma-one)
c      mass(9,9,:,:)=r_fac/(gamma-one)
c-----------------------------------------------------------------------
c     terminate.
c-----------------------------------------------------------------------
      RETURN
